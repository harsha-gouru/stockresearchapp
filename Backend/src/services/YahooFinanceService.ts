...
class YahooFinanceService {
  ...
  constructor() {
    // Start background data refresh every 5 minutes during market hours
  ...
  async getDetailedStockInfo(symbol: string): Promise<DetailedStockInfo | null> {
    try {
      ...
      const [quoteResult, summaryResult] = await Promise.all([
        ...
        }).catch(() => null) // Don't fail if summary is unavailable
      ]);
      ...
      const detailedInfo: DetailedStockInfo = {
        symbol: quoteResult.symbol || symbol,
        ...
        marketCap: this.formatMarketCap(quoteResult.marketCap),
        ...
        exchange: quoteResult.fullExchangeName || '',
        sector: profile?.sector,
        industry: profile?.industry,
        description: profile?.longBusinessSummary,
        pe: keyStats?.trailingPE || summaryDetail?.trailingPE,
        eps: keyStats?.trailingEps,
        beta: keyStats?.beta,
        dividend: summaryDetail?.dividendRate,
        yield: summaryDetail?.dividendYield ? summaryDetail.dividendYield * 100 : undefined,
  ...
  async searchStocks(query: string): Promise<Array<{ symbol: string; name: string; exchange: string }>> {
    try {
      ...
      const stocks = result.quotes
        .filter(quote => quote.typeDisp === 'Equity' && quote.symbol && quote.shortname)
  ...
  /**
   * Start background refresh for popular stocks during market hours
   */
  private startBackgroundRefresh() {
    // Refresh popular stocks every 5 minutes during market hours (9:30 AM - 4:00 PM ET, Mon-Fri)
    cron.schedule('*/5 9-16 * * 1-5', async () => {
      console.log('🔄 Refreshing popular stock data...');
      const popularSymbols = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA'];
      
      try {
        await this.getMultipleQuotes(popularSymbols);
        console.log('✅ Popular stock data refreshed');
      } catch (error) {
